...
def _blocking_download(atp_source: dict, save_path: str, timeout: int,
    ...
    while handle.status().state != lt.torrent_status.seeding:
        ...
        if status.progress >= 1.0:
            # All pieces are downloaded and hash-verified at this point (libtorrent
            # only counts a piece towards progress after it passes its hash check),
            # so every byte we need is already on disk. Waiting for the state flag
            # itself to flip to finished/seeding is just waiting on bookkeeping
            # (tracker re-announce, resume-data write, storage move) that can stall
            # indefinitely if the swarm's other peers disconnect right after we
            # finish — which is what was causing the "stuck at 100%" hang.
            break
        if time.time() - start > timeout:
